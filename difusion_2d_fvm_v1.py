...
# ============================================================
# ENSAMBLE
# Transiente : ρ·cp·ΔV/Δt  (varía por celda)
# Caras      : k_harm(k_P, k_nb) · A / d
# BC calor   : b[p] -= q_bc(t) · Δy   (resta → enfría)
# ============================================================
def ensamblar(T_iter, T_prev, t):
    ...
    for j in range(Ny):
        for i in range(Nx):
            ...
            aP   = RHO[j, i] * CP[j, i] * dx * dy / dt
            b[p] = aP * T_prev[p]

            # ---- Este ----
            if i < Nx - 1:
                k_e  = k_cell(T_iter[idx(i+1, j)], K_BASE[j, i+1])
                coef = k_harm(k_p, k_e) * dy / dx
                A[p, idx(i+1, j)] -= coef
                aP += coef
            ...
            if i > 0:
                ...
                coef = k_harm(k_p, k_w) * dy / dx
                A[p, idx(i-1, j)] -= coef
                aP += coef
            elif j < Ny // 2:
                b[p] -= q_bc(t) * dy    # extracción de calor [W/m²·m = W/m]

            # ---- Norte ----
            if j < Ny - 1:
                k_n  = k_cell(T_iter[idx(i, j+1)], K_BASE[j+1, i])
                coef = k_harm(k_p, k_n) * dx / dy
                A[p, idx(i, j+1)] -= coef
                aP += coef
...
for k_p, (t_p, T_grid) in enumerate(zip(plot_times, plot_fields)):
    ax = axes[k_p]
    ax.set_visible(True)
    c = ax.contourf(X, Y, T_grid, levels=40, cmap='hot', vmin=vmin, vmax=vmax)
    fig1.colorbar(c, ax=ax, label='T [°C]')
    ax.set_title(f't = {t_p:.0f} s')
    ax.set_xlabel('x [m]')
    ax.set_ylabel('y [m]')
    ax.axvline(Lx/2, color='white', lw=1, ls='--', alpha=0.5)   # frontera materiales
    ax.plot([0, 0], [0,    Ly/2], color='cyan', lw=4,
            label=f'q={q_bc(t_p):.0f} W/m²')
    ax.plot([0, 0], [Ly/2, Ly  ], color='gray', lw=3, label='Aislada')
    if k_p == 0:
